...
> **Aligned to:** Product Roadmap v2 (roadmap-v2.md) · Architecture v3.0 (FINAL_AI_Native_PM_Tool_Architecture_v3_Definitive.md)
...
This document is the execution plan for the AI-Native PM Tool over a 12-month development cycle. It translates the 103 features defined in the Product Roadmap v2.1 (88 original + 15 ClickUp gap features) and the architecture decisions in the Definitive System Architecture v3.0 into a sprint-by-sprint delivery schedule with concrete work items, team allocations, exit criteria, risk mitigations, and quality gates.
...
All three documents (requirements, roadmap, architecture) use a unified cross-reference scheme:
...
- **ADR-xxx** = Architecture Decision Records (from architecture v3.0 § ADR Summary)
- **RISK-xxx** = Risk Register entries (this document § 13)
- **M-xxx** = Milestones (this document § 15)
...
| FR-1900 | F-087 | Kanban Board (promoted → R1) |
| FR-1901 | F-088 | Gantt Chart (promoted → R2) |
...
Finalize all 12 ADRs (ref: architecture v3.0 § ADR Summary):
...
| 6 | RLS policies on ALL tenant-scoped tables (ref: architecture v3.0 § Tier 6) | Backend | 8h |
...
| 12 | NATS JetStream cluster setup: 6 streams as defined in architecture v3.0 § Tier 5 | Backend | 8h |
...
### Sprint R0-2 (Weeks 3-4): Auth + Tasks + Events
...
| 11 | Core NATS consumers: `audit-writer` (writes to `audit_log`), `embedding-pipeline` (placeholder — stores embedding jobs in Redis queue) | Backend | 12h |
| 12 | Next.js 15 scaffold: App Router structure, `(internal)` route group, Tailwind CSS + Shadcn UI setup, TypeScript strict | Fullstack | 16h |
| 13 | LLM Gateway skeleton: Anthropic SDK wrapper, model routing config, retry with exponential backoff, circuit breaker (5 failures → 60s open) | AI/ML | 16h |
...
| 4 | AI Orchestrator skeleton: 7-stage pipeline (ref: architecture v3.0 § 4A), stages 1-2 (TRIGGER + AUTONOMY CHECK) functional, stages 3-7 stubbed | AI/ML | 24h |
...
| # | Work Item | Owner | Est. |
|---|-----------|-------|------|
| 1 | NL to WBS Generator: 5-stage sub-pipeline (ref: architecture v3.0 § 4B) | AI/ML | 40h |
...
| 6 | "What's Next" engine (rules-based, R0): per-developer task prioritization — dependency resolved → due date → priority → assignment (ref: architecture v3.0 § 4B) | Backend | 16h |
...
| # | Work Item | Owner | Est. |
|---|-----------|-------|------|
| 1 | AI Review/Approve UI: high-density review screen for WBS proposals — approve, edit, reject per item, bulk approve/reject, keyboard shortcuts (a=approve, r=reject, e=edit, j/k=navigate) | Fullstack | 24h |
...
| 4 | Task detail view: all fields, sub-tasks, dependencies (both directions), audit history timeline, comments, AI action history | Fullstack | 16h |
| 5 | Project list view: filterable by status, sortable by name/date, project health indicator | Fullstack | 8h |
| 6 | Task list view: filterable by status/priority/assignee/phase, sortable by due date/priority/updated | Fullstack | 12h |
| 7 | "What's Next" developer view: personalized prioritized task list with AI explanations, one-click task action | Fullstack | 12h |
| 8 | Sidebar navigation: Dashboard (AI summary), Projects, Settings — active route highlighting, responsive, role-based visibility | Fullstack | 8h |
| 9 | NL project setup flow: text input → loading state → WBS review screen → approve → project created | Fullstack | 12h |
| 10 | Dashboard page: AI-generated daily summary, "What's Next" widget, recent activity | Fullstack | 8h |
...
- [ ] Task detail view shows all fields, dependencies, audit history, comments
...
| # | Work Item | Owner | Est. |
|---|-----------|-------|------|
| 1 | NL Query engine: RAG retrieval from pgvector (tenant-scoped) → context assembly → Claude Sonnet 4.5 synthesis → natural language response (ref: architecture v3.0 § 4B) | AI/ML | 20h |
| 2 | NL Query UI: text input panel, streaming response display, query history | Fullstack | 12h |
...
| 9a | @Mentions: `mentions` table, @-autocomplete dropdown in comment input, `pm.comments.mention_created` event, GET `/api/v1/users/me/mentions` endpoint | Backend + Fullstack | 8h |
| 10 | Staging environment deployment: full CDK deploy to AWS staging (mirrors prod topology) | DevOps | 16h |
| 11 | R0 Quality Gate execution: full regression test, security scan, performance baseline measurement | All | 16h |
| 12 | Documentation: API docs, dev setup guide, runbook for common operations | All | 8h |
...
- [ ] R0 Quality Gate passes (see § 14.1)
...
| 5 | AI Worker extraction: separate ECS service (`apps/ai-worker`) for async AI processing — agent loop, NATS consumers for AI, scheduled jobs (ref: architecture v3.0 § ADR-010) | DevOps + AI/ML | 16h |
...
| 2 | Projection module: internal truth → client-safe view transformation, approval workflow for client-facing content (ref: architecture v3.0 § Tier 3 — Projection Module) | Backend | 16h |
...
| 5 | Recurring tasks: `recurrence_rule` (iCal RRULE), `recurrence_parent_id`, `next_recurrence_at` columns on tasks, `recurrence-scheduler` NATS consumer, clone logic, `pm.tasks.recurrence_triggered` event | Backend | 12h |
| 6 | Assigned comments / action items: `assigned_to`, `is_action_item`, `action_status` columns on comments, PATCH `/api/v1/comments/:id/assign`, GET `/api/v1/users/me/action-items`, `pm.comments.action_assigned` event | Backend | 8h |
| 7 | Task reminders: `reminders` table, POST/GET/DELETE API, pg_cron scheduler (1-min interval), `pm.reminders.due` event, notification delivery | Backend | 8h |
| 8 | Default + custom tags: project/tenant-scoped tags with name/color, admin CRUD, tag assignment on tasks | Backend + Fullstack | 8h |
...
| # | Work Item | Owner | Est. |
|---|-----------|-------|------|
| 1 | Calendar view: month/week/day grid, tasks as colored chips on due date, drag to reschedule, view toggle integration | Fullstack | 12h |
| 2 | Table view: spreadsheet-like grid, inline editing, column resize/reorder/hide, `saved_views` table + CRUD API | Fullstack + Backend | 16h |
| 3 | Read-only Kanban board: tasks in status columns, AI annotations (blocked/priority/stalled), view toggle integration | Fullstack | 12h |
| 4 | Smart notification system: `notifications` + `notification_preferences` tables, `notification-generator` NATS consumer, bell icon + dropdown, filter by type, mark read, preferences API | Backend + Fullstack | 16h |
| 5 | Basic templates (CRUD): create project template from existing project, create project from template, template list/edit/delete | Backend + Fullstack | 8h |
| 6 | Cross-project dependency mapping: AI identifies where Project A blocks Project B, surfaces in dashboards and NL queries | AI/ML + Backend | 12h |
| 7 | Resource optimization engine: workload balancing across projects, burnout risk flags, reallocation suggestions | AI/ML | 12h |
| 8 | Full-text search: `tsvector` columns on tasks/projects/comments, GIN indexes, search API with permission scoping | Backend | 8h |
| 9 | Advanced filtering + sorting: filter by any combination (status, priority, assignee, phase, date range, tags, custom fields), saveable filter views | Backend + Fullstack | 12h |
| 10 | Dependency chain visualization: upstream/downstream graph from any task, critical path highlighted, click-through navigation | Fullstack | 16h |
| 11 | AI-annotated timeline view: tasks on time axis with AI overlays (predicted delays, at-risk milestones, resource conflicts) | Fullstack | 16h |
| 12 | Portfolio dashboard: cross-project admin view with AI curation (projects at risk, resource conflicts, blocked dependencies, delivery confidence) | Fullstack | 12h |
...
- [ ] R1 Quality Gate passes (see § 14.2)
...
| 4 | Client-facing AI assistant: NL query scoped to client permissions, answers from projected data only, human-in-the-loop filter for sensitive content (confidence < 0.8 or risk-related → flag for PM review before delivery) (ref: architecture v3.0 § Pattern 3) | AI/ML | 20h |
...
| 6 | Goals & OKRs: `goals` + `goal_task_links` tables, CRUD API, goal tree view, auto-calculated progress from linked tasks, `pm.goals.progress_updated` + `pm.goals.at_risk` events, goals sidebar nav item | Backend + Fullstack | 16h |
...
| 8 | Gantt chart view (promoted from R3): AI overlays (predicted delays, critical path, at-risk milestones), zoom levels (day/week/month), view toggle integration | Fullstack | 16h |
...
| 4 | Bulk task import (deferred from R1): CSV/XLSX upload with column mapping UI, validation, error preview, correction before commit | Backend + Fullstack | 12h |
| 5 | Calendar integration (deferred from R1): CalDAV/OAuth 2.0, external calendar sync (Google/Outlook), team member availability read | Backend | 12h |
| 6 | Form view / task intake forms: `forms` table, form builder UI, public submission endpoint, `pm.forms.submitted` event | Backend + Fullstack | 12h |
| 7 | Formula / computed fields: 'formula' field_type extension on custom_field_definitions, server-side evaluation, arithmetic + field refs + date diffs + aggregations | Backend | 8h |
| 8 | Docs & knowledge base: `documents` table, CRUD API, Markdown editor, full-text search, `pm.documents.*` events → embedding pipeline for RAG enrichment | Backend + Fullstack | 12h |
| 9 | AI writing assistant: POST `/api/v1/ai/write`, streaming response, context-aware content generation (draft, improve, summarize, translate tone) | AI/ML | 8h |
...
| 12 | R2 Quality Gate execution: most comprehensive gate — see § 14.3 and § 16.3 (Go-Live Checklist) | All | 24h |
...
**Exit Criteria:**
- [ ] R2 Quality Gate passes (see § 14.3)
- [ ] R2 Go-Live Checklist complete (see § 16.3)
...
## 7. R3 (Months 10-12): Platform + Scale — 6 Sprints
...
- [ ] R3 Quality Gate passes (see § 14.4)
...
## 9. Infrastructure Provisioning Order
...
- **Scope:** All public methods in all modules (project, task, dependency, comment, audit, user, AI orchestrator, LLM gateway, context assembly)
...
- <60% acceptance rate on any capability triggers prompt review (ref: architecture v3.0 § 4C — Evaluation Harness)
...
## 12. Task Dependencies & Critical Path

### 12.1 R0 Critical Path

```
Schema Design (R0-1, Week 1-2)
  └─→ Auth + Task CRUD + Events (R0-2, Week 3-4)
       └─→ Audit Trail + AI Foundation (R0-3, Week 5-6)
            └─→ NL→WBS + What's Next (R0-4, Week 7-8)  ⚠️ BOTTLENECK
                 └─→ UI + AI Safety (R0-5, Week 9-10)
                      └─→ NL Query + Polish (R0-6, Week 11-12)
```
...
- Backend: schema → auth → audit → WBS API → comment system
...
### 12.2 Cross-Release Dependencies
...
3. **Iterative refinement loop:** User edits to AI-generated WBS feed back into prompt tuning. When a user edits a generated WBS, log the diff (original vs edited). Aggregate edit patterns weekly to identify systematic prompt weaknesses. Example: "Users consistently add a 'QA signoff' phase that the AI misses" → update template.
...
5. **Shadow mode for production validation:** First 2 weeks of production use, NL-to-WBS runs in shadow mode (AI generates, admin reviews accuracy, not used for real projects). Only enable live mode after shadow validation confirms quality meets bar.

6. **Weekly prompt review cadence:** AI/ML engineer reviews acceptance rate, override patterns, and user feedback weekly. Prompt improvements are treated as high-priority work items, not technical debt.

7. **Fallback strategy:** If quality does not meet bar after 4 weeks of iteration, simplify to structured form with AI assistance rather than full NL-to-WBS. The fallback is: user fills in a structured template (project type, team size, key deliverables, timeline), and AI generates WBS from structured input. This is less magical but more reliable. The full NL approach is re-attempted in R1 with more training data.
...
**Mitigation:**
- Circuit breaker in LLM Gateway: 5 consecutive failures → 60s open state → cached/fallback responses (ref: architecture v3.0 § 4C)
...
**Mitigation:**
- IVFFlat index from R0, evaluate HNSW at R3 (ref: architecture v3.0 § Tier 6)
...
- Escape hatch: migrate to dedicated vector store (Pinecone/Weaviate) if needed — connection string change + migration, no application logic changes
...
**Mitigation:**
- RLS at database level: application code physically cannot query across tenants (ref: architecture v3.0 § Tier 6)
...
- Per-tenant monthly budget caps with pre-flight check (ref: architecture v3.0 § 4C)
...
**Mitigation:**
- Cut lines defined per release (§ 4-7) — know what to cut before you need to
...
**Mitigation:**
- Input sanitization before LLM context assembly (ref: architecture v3.0 § Tier 8)
...
- Schema designed for R3 from R0 (evergreen principle — ref: architecture v3.0 § What Makes This Architecture Evergreen)
...
### 14.3 R2 Quality Gate (Most Comprehensive — External Launch)
...
| M2 | Auth + CRUD complete | Week 4 | JWT auth, task CRUD, dependencies, events flowing | R0 |
...
| M11 | Client projection pilot | Week 22 | 1-2 projects shared with trusted client contact | R1 |
| M12 | **R1 Release** | **Week 24** | R1 Quality Gate passes | R1 |
...
## 16. Go-Live Checklists

### 16.1 R0 Go-Live (Internal)

- [ ] All "Cannot Cut" features functional (15 features: F-001 through F-009, F-011, F-012, F-015, F-016, F-017, F-018, F-020, F-023, F-025)
...
### 16.2 R1 Go-Live (Internal + Pilot Client)
...
### 16.3 R2 Go-Live (External Launch) — MOST COMPREHENSIVE
...
### 17.2 Feature-to-Sprint Mapping
...
### 17.3 Infrastructure Cost Schedule

Monthly infrastructure costs by release phase (ref: architecture v3.0 § Cost Model):
...
### 17.4 Key Architecture References

For detailed technical specifications, refer to the architecture document:

| Topic | Architecture v3.0 Section |
|-------|---------------------------|
| AI Orchestrator 7-stage pipeline | § Tier 4, 4A |
| AI Capabilities (all models + token profiles) | § Tier 4, 4B |
| LLM Gateway + circuit breaker | § Tier 4, 4C |
| NATS stream topology (6 streams) | § Tier 5 |
| Database schema + RLS | § Tier 6 |
| Integration adapters | § Tier 7 |
| Security controls | § Tier 8 |
| Deployment + CI/CD | § Tier 9 |
| Monitoring + observability | § Tier 10 |
...
*AI-Native PM Tool · Implementation Plan v1.1 · 12 months · 24 sprints · 103 features · 16 milestones · 12 risks · 4 quality gates*
